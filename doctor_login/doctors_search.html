<!doctype html>
<html class="no-js" lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
    <meta http-equiv="x-ua-compatible" content="ie=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title> Mdaktari| Search Results </title>
    <link rel="stylesheet" href="assets/css/foundation.css">
    <link rel="stylesheet" href="assets/css/app.css">
    <link rel="stylesheet" href="assets/css/responsive.css">
    <link rel="stylesheet" href="assets/css/font.css">
    <script src="https://use.fontawesome.com/e4e812a2f7.js"></script>

  </head>
  <body>

    <div class="title-bar" data-responsive-toggle="main-menu" data-hide-for="medium">
      <button class="menu-icon" type="button" data-toggle> </button>
      <div class="title-bar-title">Mdaktari</div>
    </div>

    <div class="top-bar primary_menu" id="main-menu">
      <div class="row">  
        <div class="top-bar-left">
          <div class="logo">
            <img src="assets/img/mdaktari_logo.png">
          </div>
          

        </div>
          <div class="top-bar-right">
            <div class="user_details">
                <img src="assets/img/avatar.png" class="user_avatar">
                <div class="user_name"> Griffin Muteti </div>
                <div class="user_points"> Logout </div>
              
            </div>
          </div>
        </div>
      </div>


    <div class="top-bar secondary_menu">
      <div class="row">  
        <div class="top-bar-left">
          <ul class="dropdown menu" data-dropdown-menu>
            <li class="menu-text">
              <a href="index.html"> My Dashboard </a>
            </li>
            <li class="menu-text">
              <a href="my_account.html"> My Account </a>
            </li>
            <li class="menu-text active">
              <a href="my_calendar.html"> My Calendar </a>
            </li>
            <li class="menu-text">
              <a href="my_appointments.html"> My Appointments </a>
            </li>
            <li class="menu-text">
              <a href="my_reviews.html"> My Reviews </a>
            </li>
          </ul>
        </div>
      </div>
    </div>

    <div class="insuarance_products">
        <div class="row">
            <form>
              <div class="large-2 columns">
              </div>
              <div class="large-8 columns collapse">
                <div class="large-11 columns">
                  <div class="input-group">
                    <input type="text" name="search_doctor" placeholder="Search Doctor" class="primary_color">
                  </div>
                </div>
                <div class="large-1 columns">  
                  <div class="input-group-button">
                      <button type="submit" class="button button_appointment">SEARCH</button>
                  </div>
                </div>
              </div>
              <div class="large-2 columns">
              </div>
            </form> 
        </div>
        <div class="row">
                     <div class="insuarance_item">
                         <div class = "row">
                             <div class="large-2 columns">
                                 <img src="assets/img/avatar2.png" class="insuarance_img">
                                 <div class="insuarance_title">  </div>
                             </div>
                             <div class="large-4 columns">
                                  <div class="insuarance_cost">
                                      <span class="doctor_title"> Dr.Faith M Vuku </span>
                                  </div>
                                  <div class="appointment_desc">
                                     <h6>
                                        Heart Specialist
                                     </h6>
                                     <p>
                                     Lorem ipsum dolor sit amet, quis nusquam no his, novum ceteros eum at. Probo partem sea id, et delenit eligendi conclusionemque usu, qui facilis splendide intellegam ei. Quo in habeo laoreet. Sea omnes verterem constituam in.
                                     </p>
                                  </div>
                             </div>
                             <div class="large-2 columns">
                                   <div class="appointment_start">
                                       JANUARY 5, 2017 9.30 AM
                                   </div>
                                   <div class="appointment_end">
                                       JANUARY 5, 2017 10.30 AM
                                   </div>
                             </div>
                             <div class="large-2 columns">
                                   <div class="insuarance_processing">
                                       
                                   </div>
                             </div>
                             <div class="large-2 columns">
                                   <a class="button button_appointment">
                                       VIEW
                                   </a>
                                   <a class="button button_appointment">
                                       MAKE APPOINTMENT
                                   </a>
                                   
                             </div>
                         </div>
                     </div>
                     <div class="insuarance_item">
                         <div class = "row">
                             <div class="large-2 columns">
                                 <img src="assets/img/avatar2.png" class="insuarance_img">
                                 <div class="insuarance_title">   </div>
                             </div>
                                     <div class="large-4 columns">
                                  <div class="insuarance_cost">
                                      <span class="doctor_title"> Dr. Faith M Vuku </span>
                                  </div>
                                  <div class="appointment_desc">
                                     <h6>
                                     Heart Specialist
                                     </h6>
                                     <p>
                                     Lorem ipsum dolor sit amet, quis nusquam no his, novum ceteros eum at. Probo partem sea id, et delenit eligendi conclusionemque usu, qui facilis splendide intellegam ei. Quo in habeo laoreet. Sea omnes verterem constituam in.
                                     </p>
                                  </div>
                             </div>
                             <div class="large-2 columns">
                                   <div class="appointment_start">
                                       JANUARY 5, 2017 9.30 AM
                                   </div>
                                   <div class="appointment_end">
                                       JANUARY 5, 2017 10.30 AM
                                   </div>
                             </div>
                             <div class="large-2 columns">
                                   <div class="insuarance_processing">
                                       
                                   </div>
                             </div>
                             <div class="large-2 columns">
                                   <a class="button button_appointment">
                                       VIEW DOCTOR
                                   </a>
                                   <a class="button button_appointment">
                                       MAKE APPOINTMENT
                                   </a>
                             </div>
                         </div>
                     </div>
                     <div class="insuarance_item">
                         <div class = "row">
                             <div class="large-2 columns">
                                 <img src="assets/img/avatar2.png" class="insuarance_img">
                                 <div class="insuarance_title">  </div>
                             </div>
                                     <div class="large-4 columns">
                                  <div class="insuarance_cost">
                                      <span class="doctor_title"> Dr. Betty G Kanini </span>
                                  </div>
                                  <div class="appointment_desc">
                                     <h6>
                                     Heart Specialist
                                     </h6>
                                     <p>
                                     Lorem ipsum dolor sit amet, quis nusquam no his, novum ceteros eum at. Probo partem sea id, et delenit eligendi conclusionemque usu, qui facilis splendide intellegam ei. Quo in habeo laoreet. Sea omnes verterem constituam in.
                                     </p>
                                  </div>
                             </div>
                             <div class="large-2 columns">
                                   <div class="appointment_start">
                                       JANUARY 5, 2017 9.30 AM
                                   </div>
                                   <div class="appointment_end">
                                       JANUARY 5, 2017 10.30 AM
                                   </div>
                             </div>
                             <div class="large-2 columns">
                                   <div class="insuarance_processing">
                                       
                                   </div>
                             </div>
                             <div class="large-2 columns">
                                   <a class="button button_appointment">
                                       VIEW
                                   </a>
                                   <a class="button button_appointment">
                                       MAKE APPOINTMENT
                                   </a>
                             </div>
                         </div>
                     </div>
                 </div>
    </div>
   
   <footer class="footer_bar">
      <div class="row">
        <div class="large-3 columns">
            <div class="logo">
              <img src="assets/img/mdaktari_logo.png">
            </div>
            <div class="footer_social_media">
              <a href="#"><i class="fa fa-facebook" aria-hidden="true"></i></a>
              <a href="#"><i class="fa fa-twitter" aria-hidden="true"></i></a>
              <a href="#"><i class="fa fa-linkedin" aria-hidden="true"></i></a>
              </div>
        </div>
        <div class="large-3 columns">
            <ul class="footer_menu">
              <li class="menu-text"><a href=""> My Dashboard </a></li>
              <li class="menu-text"><a href=""> My Account </a></li>
              <li class="menu-text"><a href=""> My Appointments </a></li>
              <li class="menu-text"><a href=""> My Reviews </a></li>
            </ul>
        </div>
        <div class="large-2 columns">
            <ul class="footer_menu">
              <li class="menu-text"><a href=""> About Us </a></li>
              <li class="menu-text"><a href=""> Contact Us </a></li>
            </ul>
        </div>
        <div class="large-3 columns">
             <ul class="footer_menu">
               <li class="menu-text"><a href=""> Privacy Policy </a></li>
               <li class="menu-text"><a href=""> Terms & Conditions </a></li>
             </ul> 
        </div>
        <div class="large-1 columns">
          
        </div>
      </div>
      <div class="copyright">
        <div class="row">
        (c) 2016 MDAKTARI
        </div>
      </div>
   </footer>
    
    <script src="assets/js/vendor/jquery.js"></script>
    <script src="assets/js/vendor/what-input.js"></script>
    <script src="assets/js/vendor/foundation.js"></script>
    <script src="assets/js/app.js"></script>    
  </body>
</html>
